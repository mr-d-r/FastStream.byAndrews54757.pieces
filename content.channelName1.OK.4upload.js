...
function get_yt_channelName() { // mr-d-r
    channelName = document.querySelector("body #header-text > #title").innerText;  //     console.log("mr-d-r:  elems979:  " + channelName);
    return channelName;
}
...
function get_yt_video_elements() {
  ...
  for (let i = 0; i < queries.length; i++) {
    ytplayer = document.querySelectorAll(queries[i]);
...
if (is_url_yt(window.location.href)) {
  const observer = new MutationObserver((mutations) => {
    ...
    if (isWatch || isEmbed) {
      get_yt_channelName();  // mr-d-r
      console.log("mr-d-r:  chanN=" + channelName);
...
document.addEventListener('click', (e) => {
  ...
  //let chanName = document.body; //window.ytInitialPlayerResponse.videoDetails.author;
  //console.log(" main: chanName=" + chanName);
